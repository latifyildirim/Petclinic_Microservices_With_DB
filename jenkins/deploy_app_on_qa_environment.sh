...
# Burada  ``values-template.yaml`` i okuyoruz ve values.yaml dosyasinin icine yaziyoruz.
...
# Burada  ``regcred`` varsa siliyor eger yoksa hata vermemesi icin sag tarafi calistiriyor
kubectl create secret generic regcred -n petclinic-qa \
    --from-file=.dockerconfigjson=/var/lib/jenkins/.docker/config.json \
    --type=kubernetes.io/dockerconfigjson
AWS_REGION=$AWS_REGION helm repo update
# Altta ``upgrade`` i ``--install`` komutunun hatasiz bir sekilde calismasini sagliyor. 
